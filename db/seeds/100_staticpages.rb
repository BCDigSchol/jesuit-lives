...
Staticpage.find_or_create_by(slug: 'project') do | page |
    ...
    <p>The Jesuit Online Necrology, launched in Summer 2021, was developed at Boston College Libraries. The database was built using open source technology and features a Ruby on Rails framework with a Blacklight discovery interface, PostgreSQL, and Solr index. All of the code used for this database is available in the Libraries’ GitHub repository. For a larger discussion of the technical aspects of the project and its visualizations, see our <a href="/pages/praxis">project praxis page</a>. Members of the development team at Boston College were: Ben Florin, Library Applications Developer; Jesse Martinez, Library Applications Developer; Matt Naglak, Digital Scholarship Library; and Allison Xu, <span style="font-family: Lato, sans-serif;">Data and Visualization Librarian</span>.</p>
...
Staticpage.find_or_create_by(slug: 'mapping') do | page |
    ...
    <ul>
        <li><a title="Link to map filtering by place of entry" href="/pages/jesuit-entry">Place of Entry Map</a>: This map allows you to quickly visualize the lives of Jesuits who took their vows in specific provinces, vice-provinces, or missions, with the ability to filter by location and by the year of interest.</li>
    </ul>
    <p style="padding-left: 40px;"><em>Possible Question</em>: <span id="docs-internal-guid-efdeb5fa-7fff-44b2-ae5c-74ac1fbd0e10">Where and when were all the Jesuit’s born who took their vows in the province of Californiae (California) in 1845?</span></p>
    <ul>
    ...
    <p>Interested in a mapping question involving the JON dataset? Feel free to contact us at <a href="mailto:jesuitportal@bc.edu">jesuitportal@bc.edu</a>.</p>
    PAGE_BODY
end
...
Staticpage.find_or_create_by(slug: 'praxis') do | page |
    ...
    page.modifiedby = current_user
    page.body = <<~PAGE_BODY
    <p><strong>Data Cleaning and Organization</strong></p>
    <p> </p>
...
Staticpage.find_or_create_by(slug: 'jesuit-entry') do | page |
    ...
    <p>The above map allows the user to filter the lives of Jesuits by the location in which they originally joined the order. Further narrowing can take place by the individual year. Green lines indicate the movement of a Jesuit from his birthplace to his place of entry, while red lines indicate the movement between the place of entry and place of death.  Each relationship type is able to be turned on or off using the filtering menu in the top-right corner. Filtered data is exportable as a .json text file. If a Jesuit within the dataset does not appear for a particular province, it is likely that the spatial coordinates for his birthplace or place of death is not known. Similarly, the precise spatial coordinates for some places listed in the <a href="/staticpages/12/edit"><em>Catalogus defunctorum</em></a> are unknown. Clicking on a Jesuit’s life-line will open a popup box with further details, as well as a link to his database entry.</p>
    <p>This map was created using the open-source javascript library <a title="Link to Leaflet home page" href="https://leafletjs.com" target="_blank" rel="noopener">LeafletJS</a> and a several plugins, including <a title="Link to Leaflet.Pancontrol plugin" href="https://kartena.github.io/Leaflet.Pancontrol/" target="_blank" rel="noopener">Leaflet.Pancontrol</a>, <a title="Link to Leaflet Markercluster plugin" href="https://github.com/Leaflet/Leaflet.markercluster" target="_blank" rel="noopener">Leaflet.MarkerCluster</a>, and <a href="https://labs.easyblog.it/maps/leaflet-search/">Leaflet.Control.Search</a>. See our <a href="/pages/praxis">project praxis page</a> for more details, or <a href="/pages/mapping">return to the map list</a>.</p>
    PAGE_BODY
end
